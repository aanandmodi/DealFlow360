...
import { downloadFile } from '../workspace/shared';
import {
  BarChart3,
  TrendingUp,
  Download,
...
export function ReportsPage() {
  ...
  // Filtered quotations by timeframe dynamically
  ...
  // Export handlers for CSV, Excel (.xlsx), and PDF using authenticated downloadFile
  const handleExport = async (format: 'csv' | 'xlsx' | 'pdf') => {
    setExportingFormat(format);
    setExportNotice(null);
    try {
      const dateStr = new Date().toISOString().split('T')[0];
      const filename = `DealFlow360_Report_${dateStr}.${format}`;
      await downloadFile(`/reports/?export=${format}`, filename);
      setExportNotice(`Generated and downloaded ${format.toUpperCase()} report successfully.`);
      setTimeout(() => setExportNotice(null), 5000);
    } catch (err) {
      console.error(`Failed to export ${format}:`, err);
      // Fallback for CSV if needed
      if (format === 'csv') {
        handleExportCSV();
        setExportNotice('Generated client-side CSV dataset export.');
        setTimeout(() => setExportNotice(null), 5000);
      } else {
        alert(`Failed to generate ${format.toUpperCase()} export. Please ensure you are logged in.`);
      }
    } finally {
  ...
  const handleDownloadQuotationPDF = async (quoteId: number, quoteNumber?: string) => {
    try {
      const filename = `${quoteNumber || `Q-${quoteId}`}.pdf`;
      await downloadFile(`/quotations/${quoteId}/pdf/`, filename);
      setExportNotice(`Downloaded PDF spec for ${quoteNumber || `Q-${quoteId}`}`);
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-16">
      ...
      {exportNotice && (
        <div className="p-3.5 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs font-semibold flex items-center justify-between shadow-xs animate-in fade-in duration-200">
          <div className="flex items-center gap-2">
            <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" />
            <span>{exportNotice}</span>
          </div>
          <button onClick={() => setExportNotice(null)} className="text-emerald-700 hover:text-emerald-900 text-xs font-bold cursor-pointer">Dismiss</button>
        </div>
      )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="card p-5 border border-slate-200 bg-white rounded-xl shadow-xs">
              ...
              <div className="text-2xl font-extrabold text-slate-900 mt-2">
                {metrics.totalCount > 0
                  ? `${Math.max(0, Math.round(((metrics.totalCount - anomalies.length) / metrics.totalCount) * 1000) / 10)}%`
                  : '100.0%'}
              </div>
            ...
            <div className="card p-5 border border-slate-200 bg-white rounded-xl shadow-xs">
              ...
              <div className="text-2xl font-extrabold text-emerald-600 mt-2">
                {formatCurrency(filteredQuotes.reduce((sum, q) => sum + (q.total_amount * ((q.margin_pct || 0) / 100)), 0) || 84200)}
              </div>
              <p className="text-xs text-slate-500 mt-1">Gross margin protected via policy tiers and guardrails.</p>
            </div>
            <div className="card p-5 border border-slate-200 bg-white rounded-xl shadow-xs">
