...
export function ApprovalDetailPage() {
  ...
  const { data: quotation, error: loadError, isLoading } = useQuery({
  ...
  if(loadError) return <Notice error={loadError}/>;
  ...
  const stage=quotation.manager_approved&&quotation.required_approval_level==='manager_finance'?'finance':'sales_manager';
  const isPending = quotation.status === 'pending_approval'&&user?.id!==quotation.sales_rep&&[stage,'admin'].includes(user?.role||'');
      ...
      {actionTaken && (
        <div className={`p-4 rounded-xl border flex items-center gap-3 shadow-sm ${
          ...
          <span className="text-xs font-bold">
            {actionTaken === 'approved' ? 'Your approval was recorded. The updated review stage is shown above.' :
             actionTaken === 'rejected' ? 'Quotation rejected. Decision saved in the activity trail.' :
