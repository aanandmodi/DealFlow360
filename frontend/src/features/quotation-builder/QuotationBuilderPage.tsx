...
export function QuotationBuilderPage() {
  ...
  const addLineMutation = useMutation({
    mutationFn: (data: { product: number; quantity: number; unit_price: number; discount_percent: number; variant?:number; subscription_plan?:number }) =>
  ...
  const handleAddProduct = useCallback((productId: number, basePrice: string) => {
    addLineMutation.mutate({ product: productId, quantity: 1, unit_price: parseFloat(basePrice), discount_percent: 0,...variant?{variant:Number(variant)}:{},...plan?{subscription_plan:Number(plan)}:{} });
    setShowProductPicker(false);
  }, [addLineMutation,variant,plan]);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
        ...
        <div>
          ...
          <div className="flex items-center space-x-3">
            <h2 className="font-outfit text-xl md:text-2xl font-extrabold text-slate-900">
              {isNew ? 'Create New Quotation' : quotation?.quote_number||'Quotation'}
            </h2>
            {quotation && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
                ...
                <div className="border-b border-slate-200 bg-slate-50/50 p-5 space-y-4">
                  <div className="flex flex-wrap gap-2">
                    {['', 'hardware', 'services', 'software', 'subscriptions'].map(cat => (
                      <button
                        key={cat}
                        onClick={() => {setSelectedCategory(cat);setVariant('');setPlan('');}}
                  ...
                  <label>Recurring plan<select value={plan} onChange={e=>setPlan(e.target.value)}><option value="">Monthly plan at catalog / tier rate</option>{products.flatMap(p=>((p as any).subscription_plans||[]).map((v:any)=><option value={v.id} key={v.id}>{v.name} · {v.cycle} · {formatCurrency(v.price)} / unit</option>))}</select></label>
                  <label>Variant (optional)<select value={variant} onChange={e=>setVariant(e.target.value)}><option value="">Standard configuration</option>{products.flatMap(p=>((p as any).variants||[]).map((v:any)=><option value={v.id} key={v.id}>{p.name} · {v.attribute}: {v.value} (+{formatCurrency(v.extra_price)})</option>))}</select></label>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-56 overflow-y-auto pr-1">
                    {products.map(p => (
                      <button
                        key={p.id}
                        disabled={addLineMutation.isPending||!!plan&&!((p as any).subscription_plans||[]).some((v:any)=>String(v.id)===plan)||!!variant&&!((p as any).variants||[]).some((v:any)=>String(v.id)===variant)}
                      ...
                      <tr key={line.id} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="px-6 py-4 text-center">
                          {canEdit ? (
                            <div className="inline-flex items-center space-x-1">
                              <button
                                aria-label={`Decrease ${line.product_name} quantity`} disabled={updateLineMutation.isPending} onClick={() => handleQuantityChange(line.id, line.quantity - 1)}
                              ...
                              <button
                                aria-label={`Increase ${line.product_name} quantity`} disabled={updateLineMutation.isPending} onClick={() => handleQuantityChange(line.id, line.quantity + 1)}
                                className="h-6 w-6 rounded border border-slate-200 flex items-center justify-center hover:bg-slate-100 text-slate-500 cursor-pointer"
                              >
                        ...
                        {canEdit && (
                          <td className="px-4 py-4 text-center">
                            <button
                              aria-label={`Remove ${line.product_name}`} disabled={deleteLineMutation.isPending} onClick={() => deleteLineMutation.mutate(line.id)}
          ...
          {!isNew && lines.length > 0 && (
            <UpsellPanel
              quotationId={Number(id)} revision={quotation?.updated_at}
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-premium space-y-3">
              ...
              <div className="space-y-2 text-xs">
                ...
                <div className="border-t border-slate-100 pt-2 flex justify-between font-bold">
                  ...
                  <span className="font-outfit text-sm text-primary font-extrabold">{formatCurrency(Number(quotation.total)+Number(quotation.tax_amount))}</span>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-premium space-y-3">
              ...
              <div className="space-y-2">
                {[
                  ...
                  { label: 'Sales Manager', active: quotation.manager_approved, needed: quotation.required_approval_level!=='none' },
      ...
      {quotation&&<><DealIntelligence id={quotation.id} revision={quotation.updated_at}/><DealConversation id={quotation.id} status={quotation.status}/></>}
    </div>
  );
}
